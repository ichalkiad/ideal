...
if __name__ == "__main__":
    ...
    dataspace = "/tmp/"
    dir_in = "{}/idealdata_expI/".format(dataspace)    
    ...
    for algo in algorithms:
        dir_out = "{}/rsspaper_expIpaperfigs_{}_nooutliers/".format(dataspace, algo)
        pathlib.Path(dir_out).mkdir(parents=True, exist_ok=True) 
            ...
            for J in Js:
                ...
                # for sigma_e in sigma_es:
                    ...
                    # for algo in algorithms:
                        ...
                        # ), secondary_y=sec_y)
                        time_allsigma_fig.add_trace(go.Box(
                            y=runtimes, showlegend=True, name=r"${{{sigma_e}}}$".format(sigma_e=sigma_e),
                            boxpoints=False, opacity=1, line=dict(color=colors[algo]),                                
                        ), secondary_y=sec_y)
                        ...
                        # ))
                        ram_avg_allsigma_fig.add_trace(go.Box(
                            y=ram["avg"], showlegend=True, name=r"${{{sigma_e}}}$".format(sigma_e=sigma_e),
                            boxpoints=False, opacity=1, line=dict(color=colors[algo]),                                
                        ))
                        ...
                        # ))
                        cpu_avg_allsigma_fig.add_trace(go.Box(
                            y=cpu_util["avg"], showlegend=True, name=r"${{{sigma_e}}}$".format(sigma_e=sigma_e),
                            boxpoints=False, opacity=1, line=dict(color=colors[algo]),                                
                        ))
                        ...
                        for param in parameter_names:
                            if param in ["X", "Z"]:
                                ...
                                #     y=theta_err[param], showlegend=True, name=r"${{{plotname}}}, \sigma_e^2={{{sigma_e}}}$".format(plotname=plotname, sigma_e=sigma_e),
                                #     boxpoints=False, opacity=1-0.2*sigma_es.index(sigma_e), line=dict(color=colors[algo])                          
                                # ))
                                param_allsigma_err_fig[param].add_trace(go.Box(
                                    y=theta_err[param], showlegend=True, name=r"${{{sigma_e}}}$".format(sigma_e=sigma_e),
                                    boxpoints=False, opacity=1, line=dict(color=colors[algo])                          
                                ))
                                ...
                                #     y=theta_err_RT[param], showlegend=True, name=r"${{{plotname}}}, \sigma_e^2={{{sigma_e}}}$".format(plotname=plotname, sigma_e=sigma_e),
                                #     boxpoints=False, opacity=1-0.2*sigma_es.index(sigma_e), line=dict(color=colors[algo])                          
                                # ))
                                param_allsigma_err_fig["{}_RT".format(param)].add_trace(go.Box(
                                    y=theta_err_RT[param], showlegend=True, name=r"${{{sigma_e}}}$".format(sigma_e=sigma_e),
                                    boxpoints=False, opacity=1, line=dict(color=colors[algo])                          
                                ))
                ...
                if plotname == "CA" or plotname == "D-MLE":
                    fix_plot_layout_and_save(time_allsigma_fig, savename, xaxis_title=r"$\sigma_e^2$", yaxis_title="Duration (in seconds)", title="", 
                                        showgrid=False, showlegend=False, 
                                        print_png=True, print_html=True, 
                                        print_pdf=True)     
                else:
                    fix_plot_layout_and_save(time_allsigma_fig, savename, xaxis_title=r"$\sigma_e^2$", yaxis_title="Duration (in minutes)", title="", 
                                        showgrid=False, showlegend=False, 
                                        print_png=True, print_html=True, 
                                        print_pdf=True)     
                # fix_plot_layout_and_save(time_allsigma_fig, savename, xaxis_title=r"$\text{Estimation algorithm, }\sigma_e^2$", yaxis_title="Duration (in minutes)", title="", 
                #                         showgrid=False, showlegend=True, 
                #                         print_png=False, print_html=True, 
                #                         print_pdf=False) 
                ...
                fix_plot_layout_and_save(ram_avg_allsigma_fig, savename, xaxis_title=r"$\sigma_e^2$", yaxis_title="Avg RAM consumption (in GB)", title="", 
                                        ...
                                        print_png=True, print_html=True, 
                                        print_pdf=True) 
                # fix_plot_layout_and_save(ram_avg_allsigma_fig, savename, xaxis_title=r"$\text{Estimation algorithm, }\sigma_e^2$", yaxis_title="Average RAM consumption (in GB)", title="", 
                #                         showgrid=False, showlegend=True, 
                #                         print_png=False, print_html=True, 
                #                         print_pdf=False) 
                ...
                fix_plot_layout_and_save(cpu_avg_allsigma_fig, savename, xaxis_title=r"$\sigma_e^2$", yaxis_title="Max CPU utilisation (% usage of 1 core)", title="", 
                                        ...
                                        print_png=True, print_html=True, 
                                        print_pdf=True) 
                # fix_plot_layout_and_save(cpu_avg_allsigma_fig, savename, xaxis_title=r"$\text{Estimation algorithm, }\sigma_e^2$", yaxis_title="Maximum CPU utilisation (% usage of 1 core))", title="", 
                #                         showgrid=False, showlegend=True, 
                #                         print_png=False, print_html=True, 
                #                         print_pdf=False) 
                for param in parameter_names:
                    ...
                    if param in ["X", "Z"]:
                        ...
                        fix_plot_layout_and_save(param_allsigma_err_fig["{}_RT".format(param)], 
                                            savename, xaxis_title=r"$\sigma_e^2$", 
                                            yaxis_title="Mean relative error (under rotation/translation)", 
                                            title="", showgrid=False, showlegend=False, 
                                            print_png=True, print_html=True, 
                                            print_pdf=True) 
                        # fix_plot_layout_and_save(param_allsigma_err_fig["{}_RT".format(param)], 
                        #                     savename, xaxis_title=r"$\text{Estimation algorithm, }\sigma_e^2$", 
                        #                     yaxis_title="Mean relative error (under rotation/translation)", 
                        #                     title="", showgrid=False, showlegend=True, 
                        #                     print_png=False, print_html=True, 
                        #                     print_pdf=False) 
                    savename = "{}/rel_err_{}_K{}_J{}_allsigmae.html".format(dir_out, param, K, J)
                    fix_plot_layout_and_save(param_allsigma_err_fig[param], savename, xaxis_title=r"$\sigma_e^2$", yaxis_title="Mean relative error", title="", 
                                        showgrid=False, showlegend=False, 
                                        print_png=True, print_html=True, 
                                        print_pdf=True)    
                    # fix_plot_layout_and_save(param_allsigma_err_fig[param], savename, xaxis_title=r"$\text{Estimation algorithm, }\sigma_e^2$", yaxis_title="Mean relative error", title="", 
                    #                     showgrid=False, showlegend=True, 
                    #                     print_png=False, print_html=True, 
                    #                     print_pdf=False) 
