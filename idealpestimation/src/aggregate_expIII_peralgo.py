...
if __name__ == "__main__":
    ...
    for K in Ks:        
        for jidx in range(len(Js)):
            ...
            time_fig = make_subplots(specs=[[{"secondary_y": False}]])
            ram_fig_max = go.Figure()
            cpu_fig_max = go.Figure()
            ram_fig_avg = go.Figure()
            cpu_fig_avg = go.Figure()
            ram_avg_allalgo_fig = go.Figure()
            cpu_avg_allalgo_fig = go.Figure()
            time_allalgo_fig = make_subplots(specs=[[{"secondary_y": True}]])
            param_allalgo_err_fig = {}
            for param in parameter_names:
                if param in ["X", "Z"]:
                    param_allalgo_err_fig["{}_RT".format(param)] = go.Figure()
                param_allalgo_err_fig[param] = go.Figure()
            for algo in algorithms:
                ...
                time_allalgo_fig.add_trace(go.Box(
                    y=runtimes, showlegend=True, name=r"${{{plotname}}}$".format(plotname=plotname),
                    boxpoints=False, opacity=1-0.2*sigma_es.index(sigma_e), line=dict(color=colors[algo]),                                
                ), secondary_y=sec_y)                          
                ram_avg_allalgo_fig.add_trace(go.Box(
                    y=ram["avg"], showlegend=True, name=r"${{{plotname}}}$".format(plotname=plotname),
                    boxpoints=None, opacity=1-0.2*sigma_es.index(sigma_e), line=dict(color=colors[algo]),                                
                ))
                cpu_avg_allalgo_fig.add_trace(go.Box(
                    y=cpu_util["avg"], showlegend=True, name=r"${{{plotname}}}-avg$".format(plotname=plotname),
                    boxpoints=False, opacity=1-0.2*sigma_es.index(sigma_e), line=dict(color=colors[algo]),                                
                ))
                for param in parameter_names:
                    if param in ["X", "Z"]:                       
                        param_allalgo_err_fig[param].add_trace(go.Box(
                            y=np.asarray(theta_err[param])/parameter_space_dim, showlegend=True, 
                            name=r"${{{plotname}}}$".format(plotname=plotname),
                            boxpoints=False, opacity=1-0.2*sigma_es.index(sigma_e), 
                            line=dict(color=colors[algo])                          
                        ))                        
                        param_allalgo_err_fig["{}_RT".format(param)].add_trace(go.Box(
                            y=np.asarray(theta_err_RT[param])/parameter_space_dim, showlegend=True, name=r"${{{plotname}}}$".format(plotname=plotname),
                            boxpoints=False, opacity=1-0.2*sigma_es.index(sigma_e), line=dict(color=colors[algo])                          
                        ))               
                    else:                        
                        param_allalgo_err_fig[param].add_trace(go.Box(
                            y=np.asarray(theta_err[param])/parameter_space_dim, showlegend=True, name=r"${{{plotname}}}$".format(plotname=plotname),
                            boxpoints=False, opacity=1-0.2*sigma_es.index(sigma_e), line=dict(color=colors[algo])                          
                        ))
                        
            
            
            savename = "{}/time_K{}_J{}_allalgo.html".format(dir_out, K, J)    
            time_allalgo_fig.update_yaxes(title_text="Duration (in seconds), D-MLE, CA", secondary_y=True)
            fix_plot_layout_and_save(time_allalgo_fig, savename, xaxis_title=r'$J={{{J}}},\sigma_e^2={{{sigma_e}}}$'.format(J=J, sigma_e=sigma_e), 
                                    yaxis_title="Duration (in minutes)", title="", 
                                    showgrid=False, showlegend=False, 
                                    print_png=True, print_html=True, 
                                    print_pdf=True)     
            savename = "{}/ram_avg_K{}_J{}_allalgo.html".format(dir_out, K, J)    
            fix_plot_layout_and_save(ram_avg_allalgo_fig, savename, xaxis_title=r'$J={{{J}}},\sigma_e^2={{{sigma_e}}}$'.format(J=J, sigma_e=sigma_e), 
                                    yaxis_title="Average RAM consumption (in GB)", title="", 
                                    showgrid=False, showlegend=False, 
                                    print_png=True, print_html=True, 
                                    print_pdf=True) 
            savename = "{}/cpu_avg_K{}_J{}_allalgo.html".format(dir_out, K, J)    
            fix_plot_layout_and_save(cpu_avg_allalgo_fig, savename, xaxis_title=r'$J={{{J}}},\sigma_e^2={{{sigma_e}}}$'.format(J=J, sigma_e=sigma_e), 
                                    yaxis_title="Max CPU utilisation (% usage of 1 core)", title="", 
                                    showgrid=False, showlegend=False, 
                                    print_png=True, print_html=True, 
                                    print_pdf=True) 
            ...
            for param in parameter_names:
                ...
                if param in ["X", "Z"]:
                    savename = "{}/rel_err_RT_{}_K{}_J{}_allalgo.html".format(dir_out, param, K, J)
                    fix_plot_layout_and_save(param_allalgo_err_fig["{}_RT".format(param)], 
                                        savename, xaxis_title=r'$J={{{J}}},\sigma_e^2={{{sigma_e}}}$'.format(J=J, sigma_e=sigma_e),
